...
# -*- coding: utf-8 -*-
"""
سكربت: جمع نتائج المباريات المنتهية من الدوري الإنجليزي (PL) وحفظها في CSV.

المصدر: football-data.org (v4)

المخرجات: ملف CSV يحتوي على:
- match_id, date, season_start, matchday, home_team, away_team, home_goals, away_goals, competition

تشغيل أمثلة:
    python collect_matches.py --league PL --start-season 2018 --end-season 2025 --output matches_data.csv
    python collect_matches.py --league PL --current-only --output matches_data.csv

ملاحظات:
- يُراعي حد 10 طلبات/الدقيقة عبر تأخير 6 ثوانٍ بين الطلبات.
- يدعم إعادة التشغيل لتحديث الملف بدون تكرار (يعتمد على match_id).
"""
import os
import time
import csv
...
DEFAULT_LEAGUE = "PL"  # أمثلة أخرى: PD (إسبانيا)، SA (إيطاليا)، BL1 (ألمانيا)، FL1 (فرنسا)
...
def get_api_key() -> str:
    ...
    if not api_key:
        raise RuntimeError(
            "FOOTBALL_DATA_API_KEY غير موجود. رجاءً اضبطه كمتغير بيئي قبل التشغيل."
        )
    return api_key


def month_ranges(start_d: date, end_d: date):
    """تجزئة المدى الزمني إلى أشهر كاملة لتفادي حدود النتائج."""
...
def season_date_range(start_season_year: int) -> tuple[date, date]:
    """
    افتراضيًا: من 1 يوليو للسنة (Y) حتى 30 يونيو للسنة (Y+1)
    هذا يغطي معظم مواسم أوروبا.
    """
...
def fetch_matches_chunk(league_code: str, start_d: date, end_d: date, api_key: str) -> list[dict]:
    """
    يجلب مباريات الدوري ضمن نافذة زمنية محددة (start_d -> end_d)
    بحالة FINISHED.
    """
    ...
    for attempt in range(5):
        ...
        if resp.status_code == 200:
            ...
            # تخطي الحدود: انتظر أكثر قليلًا
            wait_s = REQUEST_DELAY_SECONDS * (attempt + 2)
            print(f"تجاوز حد الطلبات (429). الانتظار {wait_s} ثانية ثم إعادة المحاولة...")
            safe_sleep(wait_s)
            ...
            raise RuntimeError(
                f"فشل الطلب: HTTP {resp.status_code} | رسالة: {resp.text}"
            )
...
def normalize_row(m: dict) -> dict:
    """
    تحويل سجل المباراة من JSON إلى صف مناسب للـ CSV.
    """
    ...
    row = {
        ...
        "home_team": (
            (m.get("homeTeam") or {}).get("shortName") or
            (m.get("homeTeam") or {}).get("tla") or
            (m.get("homeTeam") or {}).get("name")
        ),
        "away_team": (
            (m.get("awayTeam") or {}).get("shortName") or
            (m.get("awayTeam") or {}).get("tla") or
            (m.get("awayTeam") or {}).get("name")
        ),
        "home_goals": home_goals,
        "away_goals": away_goals,
        "competition": (
            (m.get("competition") or {}).get("code") or
...
def load_existing_ids(csv_path: str) -> set[int]:
    ...
    try:
        ...
        # في حال تلف الملف أو اختلاف الأعمدة
        return set()


def append_rows(csv_path: str, rows: list[dict]):
    # إذا الملف غير موجود، أنشئه مع رؤوس الأعمدة
...
def collect_league(
    ...
    if current_only:
        # اجمع آخر ~12 شهرًا (يغطي الموسم الجاري)
        end_d = date.today()
        ...
        # اجمع مواسم كاملة ضمن المجال المحدد
...
def parse_args():
    ...
    parser.add_argument("--league", type=str, default=DEFAULT_LEAGUE, help="رمز الدوري (افتراضي: PL)")
