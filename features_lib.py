...
def list_feature_columns() -> List[str]:
    ...
    for side in ["h", "a"]:
        ...
        features.extend([
            f"{side}_season_home_avg_gf" if side == 'h' else f"{side}_season_away_avg_gf",
...
def build_team_matches_frame(matches: pd.DataFrame) -> pd.DataFrame:
    ...
    home = m[req_cols].rename(columns={"home_team": "team", "away_team": "opponent", "home_goals": "gf", "away_goals": "ga"})
    home["is_home"] = True
    away = m[req_cols].rename(columns={"away_team": "team", "home_team": "opponent", "away_goals": "gf", "home_goals": "ga"})
    away["is_home"] = False
